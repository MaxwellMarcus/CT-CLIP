...
def upload( project ):
    ...
    for i in os.listdir( "./textout" ):
        if ".metadata." in i:
            ...
            if not session_name in project.experiments:
                os.rename( text, os.path.join( "./temp_textout", i ) )
                continue
...
for i in range( 0, 50000, 10 ):
    ...
    if True:#str( i + 1 ) in os.listdir( "./batch" ):
        ...
        inference = CTClipTextInference(
            ...
            metadata="trimmedmetadata.csv",
            num_train_steps = 1
        )
        
        pd.read_csv( "trimmedmetadata.csv" ).drop( range( 0, 10, 1 ) ).to_csv( "trimmedmetadata.csv", index=False )
        ...
        print( f"Memory use at {psutil.virtual_memory().percent} percent" )
        if psutil.virtual_memory().percent > 85:
            print( "Memory exceeded limit...\nQuiting" )
            quit()
